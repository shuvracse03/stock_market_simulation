...
def create_transaction_task(data):
    ...
    result = (
        ...
        .filter(models.StockData.ticker == transaction.ticker)
        .first()
    )
    if result is None:
    ...
    if transaction.transaction_type == "BUY":
        ...
        user_stock_qty = db.query(models.UserStockQuantity).filter(
            and_(
                models.UserStockQuantity.owner_id == user_record.id,
                models.UserStockQuantity.ticker == transaction.ticker,
            )
        )
        flag = True
        if user_stock_qty.first() is None:
            db_user_stock_quantity = models.UserStockQuantity(
                ticker=transaction.ticker,
                quantity=transaction.transaction_volume,
        ...
        user_record = user.one()
        db_user_stock_quantity = db.query(models.UserStockQuantity).filter(
            and_(
                models.UserStockQuantity.owner_id == user_record.id,
                models.UserStockQuantity.ticker == transaction.ticker,
            )
        ...
        user_stock_record.quantity = (
            user_stock_record.quantity - transaction.transaction_volume
        )
        user_record.balance += transaction_price
        stock_details.volume += transaction.transaction_volume
        stock_details.available_quantity += transaction.transaction_volume
