...
def get_stocks(db: Session):
    ...
    if cached_data:
        ...
        serialized_data = json.dumps(
            pyd_models, default=pydantic_encoder
        )  # datetime conversion needs str
...
def get_stocks_by_ticker(db: Session, ticker: str):
    ...
    if cached_data:
        ...
        print("cache miss")
        result = (
            db.query(models.StockData).filter(models.StockData.ticker == ticker).first()
        )
        if result is not None:
            pyd_model = schemas.StockData(
                id=result.id,
...
def get_transactions_by_time(
    db: Session, user_id: str, start_timestamp: str, end_timestamp: str
):
    user = db.query(models.User).filter(models.User.user_id == user_id).first()
    if user is None:
        raise HTTPException(status_code=400, detail="User does not exist")
    ...
    return (
        ...
        .filter(
            and_(
                ...
                (models.Transaction.timestamp <= end_time),
            )
        )
        .all()
    )
