...
def test_transaction_buy():
    ...
    time.sleep(0.5)
    stock = (
        db.query(models.StockData).filter(models.StockData.ticker == "GOOGLE").first()
    )
    ...
    user_stock = (
        db.query(models.UserStockQuantity)
        .filter(models.UserStockQuantity.user == user)
        .first()
    )

    assert (
        (user.balance == 49600)
        and (stock.available_quantity == 1998)
        and (transaction.transaction_volume == 2)
        and (user_stock.quantity == 2)
    )
...
def test_transaction_by_timestamp():
    user = db.query(models.User).filter(models.User.username == "shuvra3").first()
    transactions = (
        db.query(models.Transaction).filter(models.Transaction.user == user).all()
    )
    times = []
